...
using("NicheMapR", "microclima", "raster", "ggplot2", "tidyverse", "R.matlab", "matlab", "xlsx", "devtools")
...
results_all <- data.frame(Locality = rep(loc,2),  Sintopy = rep(type,2), Species =rep(c("Ihor", "Pmur"),each = 15 ), devtime = NA, birthday = NA, MonMature = NA,
                          MonRepro=NA, Fecundity=NA, Clutches=NA, AnnualAct=NA, LifeSpan=NA, Forage=NA, Bask =NA, Aspect =NA, Slope=NA, Elevation=NA, MaxTB=NA, MinTB=NA, 
                          Year=NA, Years_repro=NA, NO_Snow=NA, mean_MAX_SOLR=NA, MaxWgt=NA, MaxLen=NA, egg_dev=NA )
...
j<-"Ihor"
 ...
 # for(j in unique(results_all$Species)){
    ...
    h_die <- which(debout$P_SURV==min(debout$P_SURV))
    h_start_ihor <- 180*24
    
    
    environ_2 <- environ[h_start_ihor:h_die,]
    environ_2
    
    debout_2 <- debout[h_start_ihor:h_die,]
    debout_2
    ...
    plot(debout_2$STAGE, type="l")
    plot(environ_2$TC, type="l")
    ...
    results_all[results_all$Species == j & results_all$Locality == loc[s], "Fecundity" ] <- yearout$FECUNDITY
    results_all[results_all$Species == j & results_all$Locality == loc[s], "Clutches" ] <- yearout$CLUTCHES
    results_all[results_all$Species == j & results_all$Locality == loc[s], "AnnualAct" ] <- yearout$ANNUALACT
    results_all[results_all$Species == j & results_all$Locality == loc[s], "LifeSpan" ] <- yearout$LifeSpan
    results_all[results_all$Species == j & results_all$Locality == loc[s], "MaxTB" ] <- yearout$MAXTB
    ...
    results_all[results_all$Species == j & results_all$Locality == loc[s], "MinTB" ] <- yearout$MINTB
